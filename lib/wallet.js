module.exports = function(options) {
  ...
  var RadrAddress = require(__dirname+'/radr_address.js')({ sjcl: options.sjcl });
  ...
  RadrWallet.prototype = {
    ...
    getAddress: function(){
      ...
      return RadrAddress.fromPublicKey(publicKey);
    }
  }

  RadrWallet.getRandom = function(){
    var secretKey = MasterKey.getRandom().value;
    return new RadrWallet(secretKey);
  };

  RadrWallet.generate = function() {
    /* Generate a 128-bit master key that can be used to make 
       any number of private / public key pairs and accounts
    */
    var secretKey = MasterKey.getRandom().value;
    var wallet = new RadrWallet(secretKey);
    return {
